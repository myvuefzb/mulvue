...
export default {
  ...
  methods: {
    ...
    /* 银行卡号拍照事件方法 */
    photoHandle () {
      Native.NUI.openCamera({"callback":"getBankCardPhotoHandle",belong:''})
    },
    ...
    submit () {
      ...
      confirmSign(postData).then(function (data) {
        ...
        if (data && parseInt(data.ret) === 0) {
          ...
          Native.NRouter.open({url:'common-result.html',data: {title:'绑卡结果', type: 'card',status: data.data.status}})
        } else {
          _self.$vux.toast.show({
            type: 'text',
            text: data.message,
  ...
  mounted () {
    ...
    window.platformAdapter.getBankCardPhotoHandle = (data)=> {
